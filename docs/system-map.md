...
| `agent-delta-stats` | Explicit feedback aggregate inspection | `--limit`, optional `--review-only`, optional `--review-category`, optional `--source`, optional `--entity`, optional `--state` | None | Emits aggregate counts over the same bounded recent committed-delta slice as `agent-delta-list`: counted deltas, review-required count, counts by outcome state, counts by normalized `agent-delta:` source, and counts by review-required category. The command accepts the same combined filters, including CLI-normalized `--review-category` and `--state` input, rejects malformed `--limit` / `--source` / `--entity` values and unknown state/review-category filters before workspace setup, hard-caps requested limits and filtered scans to 100 rows, and omits summaries, entities, protocol node ids, raw text, hashes, vector blobs, scores, and internal SQLite row ids. `agent-delta-summary` is accepted as an alias. |
| `agent-delta-edges` | Explicit feedback graph inspection | `--limit`, optional `--node-id`, optional `--relationship`, optional `--review-only`, optional `--review-category`, optional `--source`, optional `--entity`, optional `--state` | None | Emits bounded graph edges touching committed agent-delta nodes as JSON with protocol-facing source and target `node_id` values, relationship type, clamped edge weight, and UTC created/updated timestamps. It can first scope committed delta nodes with the same review/review-category/source/entity/state filters as list/stats, then filter to one UUIDv4-shaped protocol node id normalized to canonical lowercase or one bounded CLI-normalized relationship type, rejects missing or malformed node-id filters, malformed limit/source/entity/relationship filters, and unknown state/review-category filters before workspace setup, hard-caps output at 100 rows, and omits raw text, summaries, hashes, vectors, scores, and internal SQLite row ids. |
