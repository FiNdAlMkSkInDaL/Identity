...
| `Prototype Memory Graph Edges` | Local store table | Implemented prototype | `crates/identityd/src/identity.rs` | Stores bounded weighted edges in `graph_edges`, rejects invalid/self edges, uses SQLite foreign-key checks, auto-links nearby vectors during memory promotion, exposes manual edge commands, reports graph health including agent-delta node, outcome-edge, conflict-edge, and supersession-edge counts, applies explicit edge-weight decay, exports bounded protocol-safe agent-delta edge JSON using endpoint `node_uid` values and canonical relationship labels with `UNKNOWN` for malformed legacy rows, and automatically decays older agent-delta outgoing non-supersession edges when a newer same-entity delta supersedes them. This is local graph scaffolding inside the SQLite `.me` prototype, not final LanceDB graph completion. |
...
| `Explicit Agent Delta Boundary` | Feedback primitive | Implemented prototype | `crates/identityd/src/delta.rs`, `crates/identityd/src/main.rs`, `crates/identityd/src/identity.rs` | Provides `agent-delta-schema`, `agent-delta-validate`, `agent-delta-preview`, and `agent-delta-commit` for user-provided agent outcome text or reviewed candidate JSON. The boundary prints the local candidate contract with schema version, source prefix, allowed outcome states, review categories, field limits, validation rules, and a candidate template; normalizes prose sources as bounded lowercase slugs under `agent-delta:`; applies the shared ingest safety filter plus outbound security blacklist; extracts or parses a bounded deterministic candidate with schema version, outcome state, summary, obvious entities, key/value attributes, `requires_review`, and review-required categories; rejects unknown candidate JSON fields, reviewed summary/entity/attribute-value strings that are untrimmed or not single-line, reviewed JSON that omits sensitive review categories inferred from its outcome state, summary, entities, or attributes, duplicate single-value CLI fields, and external `--source` overrides for candidate JSON; validates the candidate against the local delta schema and safety gates before JSON preview or memory commit; can report compact validation status without echoing payload fields; and commits only that candidate into `.me` memory when explicitly requested. Finance, health, legal identity, and private-communication markers require `--allow-sensitive` before commit, and repeated identical deltas dedupe through a stable negative cleaned-event id before embedding/vector writes. Committed deltas use concise source-specific summary tokens, structured attributes for outcome/source/summary/entities/review categories/delta attributes, a protocol-safe bounded list export, a protocol-safe single-node show export by `node_uid`, an aggregate stats export by outcome state/source/review category, and a bounded graph-edge export using protocol-facing endpoint ids. A bounded same-entity graph reconciliation transaction links outcomes to recent matching local memories with `OUTCOME_FOR` / `UPDATED_BY` edges, links newer matching deltas to older deltas with `SUPERSEDES` / `SUPERSEDED_BY`, adds `ATTRIBUTE_CONFLICTS_WITH` / `ATTRIBUTE_REPLACED_BY` edges when same-key attributes change, and decays the older delta's outgoing non-supersession edges using the documented edge-decay formula. This is Phase 3 scaffolding for write-back; it is not an ambient session watcher or browser/API observer. |
...
| :--- | :--- | :--- | :--- | :--- |
| `--help` / `-h` / `help` | CLI inspection | None | None | Prints command help before workspace setup, so asking for help does not create or touch the default Identity workspace. |
...
| `agent-delta-list` | Explicit feedback inspection | `--limit`, optional `--review-only`, optional `--review-category`, optional `--source`, optional `--entity`, optional `--state` | None | Emits recent committed agent deltas as bounded JSON with protocol-facing node ids, UTC timestamps, canonicalized source, top-level outcome state, extracted entities, extracted delta attributes, summary, top-level review flags/categories, and structured attributes reconstructed from bounded agent-delta labels rather than arbitrary stored metadata. It can filter to review-required deltas with `--review-only`, to one CLI-normalized review-required category with `--review-category`, to one normalized `agent-delta:` source with `--source`, to one extracted entity with `--entity`, and to one CLI-normalized outcome state with `--state` so lowercase, kebab-case, and whitespace-separated input maps to the strict stored uppercase state labels; it omits raw text, hashes, vector blobs, scores, and internal SQLite row ids, rejects malformed or duplicate `--limit` / `--source` / `--entity` values and unknown or duplicate state/review-category filters before workspace setup, and clamps requested limits and filtered scans to 100 rows. |
| `agent-delta-show` | Explicit feedback inspection | `--node-id` | None | Requires a UUIDv4-shaped protocol node id, normalizes it to canonical lowercase before lookup, and emits exactly one committed agent delta by protocol-facing `node_uid`, using the same protocol-safe JSON shape as `agent-delta-list`. Missing or malformed node ids are rejected before workspace setup, unknown ids and ordinary non-agent memory nodes do not resolve, and output omits raw text, hashes, vector blobs, scores, and internal SQLite row ids. |
| `agent-delta-stats` | Explicit feedback aggregate inspection | `--limit`, optional `--review-only`, optional `--review-category`, optional `--source`, optional `--entity`, optional `--state` | None | Emits aggregate counts over the same bounded recent committed-delta slice as `agent-delta-list`: counted deltas, review-required count, counts by outcome state, counts by normalized `agent-delta:` source, and counts by review-required category. The command accepts the same combined filters, including CLI-normalized `--review-category` and `--state` input, rejects malformed or duplicate `--limit` / `--source` / `--entity` values and unknown or duplicate state/review-category filters before workspace setup, hard-caps requested limits and filtered scans to 100 rows, and omits summaries, entities, protocol node ids, raw text, hashes, vector blobs, scores, and internal SQLite row ids. |
| `agent-delta-summary` | Explicit feedback aggregate inspection | Same as `agent-delta-stats` | None | First-class CLI alias for `agent-delta-stats`; it emits the same protocol-safe aggregate JSON, accepts the same bounded filters, remains capped at 100 rows, and uses its own command label for preflight errors before workspace setup. |
| `agent-delta-edges` | Explicit feedback graph inspection | `--limit`, optional `--node-id`, optional `--relationship`, optional `--review-only`, optional `--review-category`, optional `--source`, optional `--entity`, optional `--state` | None | Emits bounded graph edges touching committed agent-delta nodes as JSON with protocol-facing source and target `node_id` values, canonical relationship type, clamped edge weight, and UTC created/updated timestamps, using `UNKNOWN` for malformed legacy relationship labels. It can first scope committed delta nodes with the same review/review-category/source/entity/state filters as list/stats-summary, then filter to one UUIDv4-shaped protocol node id normalized to canonical lowercase or one bounded CLI-normalized relationship type, rejects missing, duplicate, or malformed node-id filters, malformed or duplicate limit/source/entity/relationship filters, and unknown or duplicate state/review-category filters before workspace setup, hard-caps output at 100 rows, and omits raw text, summaries, hashes, vectors, scores, and internal SQLite row ids. |
| `agent-delta-schema` | Explicit feedback schema inspection | None | None | Runs before workspace setup and emits the local reviewed-candidate contract as JSON, including schema version, `agent-delta:` source prefix, default source, allowed outcome states, allowed review categories, field limits, validation rules, and a strict candidate template suitable for review before `--candidate-json` or `--candidate-json-stdin`. |
| `agent-delta-validate` | Explicit feedback validation | `--text` or `--content` or `--stdin`, optional `--source`; or `--candidate-json` / `--candidate-json-stdin` | None | Runs before workspace setup; rejects missing candidate input values, including a following flag where `--text`, `--content`, `--source`, or `--candidate-json` expected a value, rejects duplicate candidate input flags, rejects combined candidate input modes so text, stdin, and candidate JSON paths remain mutually exclusive, and rejects `--source` with candidate JSON because reviewed JSON carries its own source field; applies the same candidate extraction/parsing, unknown-field rejection, schema checks, under-declared sensitive-category rejection, ingest safety filter, and outbound security blacklist as preview/commit; then emits compact JSON with `valid=true`, source, outcome state, review flags/categories, whether commit would require `--allow-sensitive`, and entity/attribute counts. It does not write to `.me` and does not echo summaries, entities, attributes, raw text, hashes, vectors, scores, or internal SQLite row ids. |
| `agent-delta-preview` | Explicit feedback candidate extraction | `--text` or `--content` or `--stdin`, optional `--source`; or `--candidate-json` / `--candidate-json-stdin` | None | Runs before workspace setup; rejects missing candidate input values, including a following flag where `--text`, `--content`, `--source`, or `--candidate-json` expected a value, rejects duplicate candidate input flags, rejects combined candidate input modes so text, stdin, and candidate JSON paths remain mutually exclusive, and rejects `--source` with candidate JSON because reviewed JSON carries its own source field; extracts a bounded deterministic agent outcome candidate from user-provided text after shared ingest safety and outbound security blacklist checks, or validates a reviewed candidate JSON object without writing. Validates the candidate schema, rejects unknown JSON fields and reviewed JSON that under-declares inferred sensitive review categories, then prints JSON only, including `schema_version`, `requires_review`, and review-required categories. |
| `agent-delta-commit` | Explicit feedback write-back | `--text` or `--content` or `--stdin`, optional `--source`; or `--candidate-json` / `--candidate-json-stdin`; optional `--allow-sensitive`, optional `--json` | `identity.me/state.db`, `identity.me/vectors/node-*.f32le`, `identity.me/state.db.graph_edges` | Rejects missing candidate input values, including a following flag where `--text`, `--content`, `--source`, or `--candidate-json` expected a value, rejects duplicate candidate input flags, rejects combined candidate input modes, and rejects `--source` with candidate JSON before workspace setup. Extracts or parses the same schema-validated agent outcome candidate and applies the sensitive review gate before workspace setup, then commits it through the existing `.me` memory insertion path under `agent.outcome/AGENT_DELTA`. Sensitive review categories fail closed unless `--allow-sensitive` is passed, duplicate retries reuse the same stable local delta id before embedding/vector writes, the command reports the protocol-facing node id rather than the internal SQLite row id, prints `write_status=created` or `write_status=existing`, can emit a minimal protocol-safe commit receipt as JSON with `--json` without summary/entity/attribute payload fields, and bounded same-entity graph reconciliation links the outcome to existing local context, marks changed same-key attributes, and decays superseded older outcome edges. This is user-triggered write-back, not ambient session watching. |
...
| Explicit agent outcome to `.me` | User-triggered `agent-delta-preview` / `agent-delta-commit` | Shared safety filter, outbound security blacklist, deterministic delta extractor or strict candidate-JSON parser, delta schema validator, sensitive-category review gate, identity memory store, bounded graph reconciliation | Preview emits or validates a bounded schema-versioned candidate without writing. Commit validates the candidate and sensitive-category review gate before workspace setup, stores only the validated candidate through the existing local embedding/vector/protection path under `agent.outcome/AGENT_DELTA`, requires `--allow-sensitive` for sensitive review categories, dedupes identical retries through the stable delta id before vectorization, reports created versus existing writes through a minimal protocol-safe JSON receipt when requested, links same-entity outcomes to recent local memory graph nodes, records changed same-key attributes as explicit conflict/replacement graph edges, and decays older superseded outcome evidence without deleting history. `agent-delta-list`, `agent-delta-stats` / `agent-delta-summary`, and `agent-delta-edges` inspect the committed boundary through bounded protocol-safe JSON. This does not observe sessions automatically. |
