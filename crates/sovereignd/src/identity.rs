...
impl SqliteMemoryBackend {
    fn open(paths: &SovereignPaths, embedding: &EmbeddingEngine) -> Result<Self, IdentityError> {
...
mod tests {
    ...
    fn windows_activity_summary_prioritizes_structured_context() {
        let summary = summarize_windows_activity(
            "Active application: Code.exe\nActive window title: Identity - README.md\nFocused control text: Search files\nVisible window text:\n- Sovereign local-first notes\n- Another line",
        );

        assert_eq!(
            summary,
            "UI activity in Code.exe; window Identity - README.md; focus Search files; visible Sovereign local-first notes"
        );
    }
    ...
    fn windows_activity_attributes_extract_structured_fields() {
        ...
        assert_eq!(
            attributes,
            "{\"application\":\"Code.exe\",\"window_title\":\"Identity\",\"focused_text\":\"Search files\",\"first_visible_text\":\"Sovereign note\"}"
        );
    }
    ...
    fn preserves_source_specific_memory_classification() {
        let root = std::env::temp_dir().join(format!(
            "sovereign-identity-classification-test-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_nanos()
    ...
    fn promotion_syncs_vector_blob_into_vector_store_root() {
        let root = std::env::temp_dir().join(format!(
            "sovereign-vector-store-sync-test-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_nanos()
    ...
    fn reopening_identity_store_backfills_missing_vector_store_files() {
        let root = std::env::temp_dir().join(format!(
            "sovereign-vector-store-backfill-test-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_nanos()
